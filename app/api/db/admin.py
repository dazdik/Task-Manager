...
class UserModelView(ModelView, model=User):
    column_list = [User.id, User.username, User.password, User.email, User.role, User.created_at]
    column_sortable_list = [User.id]
    column_searchable_list = [User.username, User.role]


class TaskModelView(ModelView, model=Task):
    column_list = [Task.id, Task.name, Task.description, Task.created_at, Task.status, Task.urgency]
...
class UserTasksAssociationModelView(ModelView, model=UserTasksAssociation):
    column_list = [UserTasksAssociation.user,
